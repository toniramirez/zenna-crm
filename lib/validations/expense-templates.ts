...
/** Edit an existing template payment (row in `expenses` with template_id). */
export const editTemplatePaymentSchema = z.object({
  expenseId: z.string().uuid("Falta el pago."),
  period: z
    .string()
    .regex(/^\d{4}-\d{2}-\d{2}$/, "Período inválido."),
  amount: z
...
export type EditTemplatePaymentInput = z.infer<
  typeof editTemplatePaymentSchema
>;
